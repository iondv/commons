...
const { resources: { ResourceStorage, Share } } = require('@iondv/commons-contracts');
    ...
    return checkDest(fn)
      .then(check =>
        new Promise((resolve, reject) => {
          mkdirp(path.join(_options.storageBase, check.path))
            .catch(reject)
            .then(() => {
              ...
              if (typeof d === 'string' || typeof d.pipe === 'function') {
                ...
                writer.on('finish', () => {
                  resolve(path.join(check.path, check.filename));
                });
                reader.pipe(writer);
              } else {
