 ...
 */
'use strict';
const ProfilePlugin = require('@iondv/accounts-contracts/contracts/ProfilePlugin');
...
function CustomProfile(options = {}) {
  ...
  if (!options.metaRepo || !options.dataRepo) {
    throw new Error(t('Failed to refer dependecy components'));
  }
  ...
  this._validate = function (data) {
    ...
    for (let key of Object.keys(fields)) {
      if (fields[key] && fields[key].required) {
        if (!data[key]) {
          return Promise.reject(new Error(t('Required fields are not set.')));
        }
      }
