...
const binDir = path.normalize(path.join(process.cwd(), 'node_modules', 'bin'));
          ...
          for (let nm in jobs) {
            if (jobs.hasOwnProperty(nm) && statuses.hasOwnProperty(nm)) {
              ...
              if (
                ...
                runned[nm] = child.fork(path.join(binDir, 'job-runner'), [nm], {stdio: ['pipe', 'inherit', 'inherit', 'ipc']});
              ...
              if (status === Scheduler.statusCodes.MANUALLY_STARTING && !runned[nm]) {
                runned[nm] = child.fork(path.join(binDir, 'job'), [nm], {stdio: ['pipe', 'inherit', 'inherit', 'ipc']});
                runned[nm].on('exit', () => {
                  delete runned[nm];
